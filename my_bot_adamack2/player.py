...
class Player(Bot):
    ...
    def get_action(self, game_state, round_state, active):
        ...
        if (CallAction in legal_actions or RaiseAction in legal_actions):
            ...
            hand_strength = hand_strength + 1*self.opp_bluff_pct - 0.09*street*(opp_pip/(opp_pip+opp_stack))
        ...
        # Flop play
            ...
            # Opponent raised or we checked pre-flop
                ...
                # If villain checks or it is our turn to act, bet
                    ...
                    my_action = RaiseAction(max(min_raise, min(potsize//3, max_raise, my_stack)))

                # Bluff raise
                elif (random.random() <= 0.3 and hand_strength >= 0.2 and hand_strength <= 0.6):
        ...
        # Turn play
            ...
            # Reraise a bet
            if hand_strength >= 0.85 and RaiseAction in legal_actions:
                my_action = RaiseAction(max(min_raise, min(2*continue_cost, max_raise, my_stack)))
            ...
            elif hand_strength > foldfreq and CallAction in legal_actions:
                my_action = CallAction() 
            
            # Bluff raise
            elif (random.random() <= 0.3 and RaiseAction in legal_actions and hand_strength >= 0.2 and hand_strength <= 0.6):
                my_action = RaiseAction(max(min_raise, min(2*potsize//3, max_raise, my_stack)))
        ...
        # River play
            ...
            if RaiseAction in legal_actions and hand_strength >= 0.7:
                print("I think we have it. Value bet")
            ...
            # Bluff sometimes
            if RaiseAction in legal_actions:
                if random.random() >= (raiseamount / (potsize + raiseamount)) and hand_strength <= 0.4:
                    print("We're bluffing here")
                    my_action = RaiseAction(raiseamount)
